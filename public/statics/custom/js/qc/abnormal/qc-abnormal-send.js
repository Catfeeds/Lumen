    ...
    data.forEach(function(item,index){
        var tr=`
            <tr class="tritem" data-id="${item.id}">
                ...
                <a class="link_button" style="border: none;padding: 0;" href="/QC/abnormalSendList?id=${item.id}"><button data-id="${item.id}" class="button pop-button send">发送相关部门</button></a>
                <button data-id="${item.id}" class="button pop-button closure">异常结案</button></td>
            </tr>
        `;
        ele.append(tr);
...
//搜索物料属性
    ...
    $('.table_tbody').on('click','.closure',function(){
        $(this).parents('tr').addClass('active');
        console.log('123');
        abnormalClosureModel($(this).attr("data-id"));
    });
...
function abnormalClosureModel(id){
