    ...
    data.forEach(function(item,index){
        var tr=`
            <tr class="tritem" data-id="${item.id}" >
                ...
                <td class="right">
                    ${item.result==1&&item.audit_status==0?`<button data-id="${item.id}" class="button pop-button result_audit">审核</button>`:item.result==1&&item.audit_status==1?`<button data-id="${item.id}" class="button pop-button result_audit_back">反审</button>`:''}
                    <button data-id="${item.id}" data-code="${item.material_code}" class="button pop-button claim">索赔</button>
...
function bindEvent(){
    ...
    //取消
    $('body').on('click', '#claim_form .cancle', function (e) {
        e.stopPropagation();
        layer.close(layerModal);
...
//     urlLeft+="&page_no="+pageNo+"&page_size="+pageSize;
//     AjaxClient.get({
//             url: URLS['check'].export+"?"+_token+urlLeft,
//             dataType: 'json',
//             beforeSend: function(){
//                 layerLoading = LayerConfig('load');
//             },
    ...
    AjaxClientSap.get({
        ...
        fail: function(rsp){
            ...
            if(typeof(rsp.DATA) != "undefined"){
                layer.confirm("SAP推送失败！错误日志为："+rsp.DATA.RESPONSE.SUBRETURNINFO, {icon: 5, title:'提示',offset: '250px',end:function(){
                    $('.uniquetable tr.active').removeClass('active');
                }}, function(index){
                    layer.close(index);
                    getChecks();
                ...
                layer.confirm("推送失败！错误日志为："+rsp.RETURNINFO, {icon: 5, title:'提示',offset: '250px',end:function(){
                    $('.uniquetable tr.active').removeClass('active');
                }}, function(index){
                    layer.close(index);
                    getChecks();
...
function sumbitAudit(id) {
    AjaxClient.get({
        url: URLS['check'].audit+"?"+_token+"&id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
...
function sumbitAuditBack(id) {
    AjaxClient.get({
        url: URLS['check'].auditBack+"?"+_token+"&id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
        success: function(rsp){
            layer.close(layerLoading);
            layer.confirm('反审成功！', {icon: 1, title:'提示',offset: '250px',end:function(){
                $('.uniquetable tr.active').removeClass('active');
            }}, function(index){
                layer.close(index);
                getChecks();
...
function updateStatus(id) {
    AjaxClient.get({
        url: URLS['check'].updateStatus+"?"+_token+"&check_id="+id,
        dataType: 'json',
        beforeSend: function(){
            layerLoading = LayerConfig('load');
        },
