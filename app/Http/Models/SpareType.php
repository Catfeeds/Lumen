...
class SpareType extends Base
{
	public function __construct()
    {
        $this->table='ruis_spare_type';
      
        $this->aliasTable=[
            'sparetype'=>$this->table.' as sparetype',
        ];
    }
    ...
    {
        $data = [
            'sparetype.id          as    object_id',
            'sparetype.name        as    object_name',
            'sparetype.code        as    object_code',
            'sparetype.sort        as    object_sort',
            'sparetype.parent_id   as    object_parent_id',
            'sparetype.remark      as    object_remark',
        ];

        $obj = DB::table($this->aliasTable['sparetype'])
            ->select($data)
            ->where("sparetype.$this->primaryKey",'=',$id)
            ->first();
