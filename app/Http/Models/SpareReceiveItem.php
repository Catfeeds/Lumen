...
class SpareReceiveItem extends  Base
{
    public function __construct()
    {
        $this->table='ruis_spare_receive_item';

        //定义表别名
        $this->aliasTable=[
            'receiveitem'=>$this->table.' as receiveitem',
        ];
    }
	...
	public function saveItem($input, $order_id)
	{
		foreach (json_decode($input['items'],true)  as  $key=>$item)
		{
			 $item_data = [
                    ...
                    'spare_id'     => isset($item['spare_id'])? $item['spare_id']:""   ,
                    'trade_id'     => -1,       //  -1 :出库
                ];
	...
	{
		$list = $this->getLists([['receive_id','=',$id]], 'id');
		foreach ($list as $val) 
			$ids[] = $val;
	...
	{
		$list = $this->getListsByWhere([['receive_id','=',$id]]);
