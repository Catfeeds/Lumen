...
class SparePur extends  Base
{
    public function __construct()
    {
        $this->table='ruis_spare_purchase';
        $this->item_table='ruis_spare_purchase_item';
        ...
        //定义表别名
        $this->aliasTable=[
            'purchase'=>$this->table.' as purchase',
        ...
        if(empty($this->purchaseitem)) $this->purchaseitem =new SparePurItem();
    ...
    public function update($input)
    {
        ...
        try {
            ...
            //2、入库明细添加
            $this->purchaseitem->saveItem($input,$reveive_id);
    ...
    public function add($input)
    {
        ...
        try {
            ...
            $data=[
                ...
                'employee_id'=>$input['employee_id'],
                'reason'=>$input['reason'],
                'arrival_time'=>  strtotime($input['arrival_time']),
                'creator'=>$creator_id,
                'remark'=>$input['remark'],
                'ctime'=>time(),
            ];
            ...
            //2、添加明细
            $this->purchaseitem->saveItem($input, $insert_id);
    ...
    public function getList($input)
    {
        ...
        $obj_list = DB::table($this->aliasTable['purchase'])
            ->orderBy('id','asc')
            ->select(
                'purchase.id',
                'purchase.ctime',
                ...
                'purchase.remark')
            ->leftJoin($this->aliasTable['employee'], 'purchase.employee_id', '=', 'employee.id')
            ->leftJoin($this->aliasTable['creator'], 'purchase.creator', '=', 'creator.id')
            ->leftJoin($this->aliasTable['auditor'], 'purchase.auditor', '=', 'auditor.id')
            ->leftJoin($this->aliasTable['department'], 'purchase.department_id', '=', 'department.id')
    ...
    public function audit($input)
    {
        ...
        // 获取明细 数据
        $gdata = $this->purchaseitem->getItems($order_id);
        ...
        try{
            ...
            foreach ($gdata as $value) {
                ...
                // 外键字段关联
                $this->purchaseitem->save(array('item_id'=>$item_), $item_id);
    ...
    public function noaudit($input)
    {
        ...
        try{
            ...
            foreach ($gdata as $value) {
                ...
                //给明细的 item_id  重新置空
                $this->purchaseitem->save(array('item_id'=>"NULL"), $vaule_arr['id']);
    ...
    public function getPageList($input)
    {
        ...
        $builder = DB::table($this->aliasTable['purchase'])
            ->select(
                'purchase.id',
                'purchase.ctime',
                'purchase.code',
            ...
            $obj_list->total_count = DB::table($this->aliasTable['purchase'])->where($where)->count();
            return $obj_list;
    }
    ...
    public function getItemsByOrder($id)
    {
        ...
        $obj_list = DB::table($this->aliasTable['item'])
            ...
            ->where('item.spare_purchase_id', $id)
